...
class MessageController extends Controller
{
    ...
    public function index($id){
        ...
        // $messages = Message::with('user')->where('conversation_id', $id)->limit(5)->get();
    ...
    private function makeMessagesReaded( $conversation_id ){
            ...
            Resipient::where('user_id',Auth::id())->whereIn('message_id',$a)->where('read_at',null)->update(['read_at'=>now()]);
            return ;
    ...
    public function store(Request $request){
        ...
        try {
            ...
            if (!$conversation) {
                // $name=$type=='peer'?$receiver_user->name:'group';


                $conversation = Conversation::create([
                    'user_id' => Auth::id(),
                    'type' => $type,
                    'img'=>$receiver_user->img
                ]);
